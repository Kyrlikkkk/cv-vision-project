...
RESULTS_DIR = Path("results/faster_rcnn_official")
RESULTS_DIR.mkdir(parents=True, exist_ok=True)
DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

#Загрузка модели
model = load_faster_rcnn(num_classes=11)
model.load_state_dict(torch.load("results/faster_rcnn/model.pth", map_location=DEVICE))
model.to(DEVICE)
model.eval()
print("Модель Faster R-CNN загружена")
...
try:
    ...
    print("\nРезультаты Faster R-CNN (COCOeval):")
