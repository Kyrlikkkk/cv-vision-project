...
RESULTS_DIR = Path("results/ssd_official")
RESULTS_DIR.mkdir(parents=True, exist_ok=True)
DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

model = load_ssd(num_classes=11)
model.load_state_dict(torch.load("results/ssd/model.pth", map_location=DEVICE))
model.to(DEVICE)
model.eval()
print("Модель SSD загружена")
